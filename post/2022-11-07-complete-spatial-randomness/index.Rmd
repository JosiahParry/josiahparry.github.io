...
date: '2022-11-07'
slug: []
categories: [spatstats, rstats, sfdep]
tags: []
draft: no
---

< this is a cliche about Tobler's fist law and things being related
in space>. Because of Tobler's first law, data tend to not follow any
specific distribution. So, p-values are sort of...not all that accurate.
P-values in spatial statistics take a "non-parametric" approach instead
of an "analytical" one. 

Let's compare to the t-test. T-tests make the assumption that data are 
...
In the spatial case, our null hypothesis is generally "is the observed 
...
Load the super duper cool packages. We create queen contiguity neighbors.
From our neighbor object, we can create row-standardized weights.
...
a limitation, however. It is that there are only 2499 
possible neighborhood configurations that we can sample that are 
not our own. 

Here we visualize a random point and it's neighbors.

```{r }
# block sampling for bootstrap
# For bootstrap we sample randomly from the existing distribution
...
number of simulations—typically 999. In our case we should be okay. 
But as the simulations increase in size, we are limited in the amount of 
samples we can draw. The number of neighborhoods becomes limiting! 
It's limiting because after we reach n - 1 simulations, we've ran out and
we begin to resample our neighborhood configurations. 
...
`obs`. We'll need this to calculate a simulated p-value.
...
~0.56 is a fair amount of positive spatial autocorrelation indicating 
...
value under spatial randomness using the boostrapped sampling. To do so,
we boostrap sample our neighbors, recalculate the weights and then 
the global Moran.
Now, if you've read my vignette on conditional permutation, you know what
...
the funciton `replicate()` for these purposes. 

```{r }
nsim = 499 
```

> _Also, a thing I've started doing is assigning scalars / constants with an equal sign because they typically end up becoming function arguments._
...
See this? There are only 144 unique value! That isn't much! 
